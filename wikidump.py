...
class WikiDump(object):
    ...
    def text(self):
        if not self._text:
            text = ''
            sentences = self._get_sentences_from_paragraphs_soup(paragraph_wise=True)
    ...
    def _get_sentences_from_paragraphs_soup(self, paragraph_wise=False):
        """Get all sentences from all paragraphs in the given
           ...
           Exclude the first sentence due to cascaded html tags.
        """
        ...
        for paragraph_soup in self._soup:
            sentences_in_paragraph = self._get_sentences_from_paragraph_soup(paragraph_soup)

            # Add all sentences to the result list.
            sentences_in_paragraphs = sentences_in_paragraphs + [seperator] + sentences_in_paragraph
...
if __name__ == '__main__':
    ...
    urls = ['https://en.wikipedia.org/wiki/Fashion']

    wikidump = WikiDump(urls[0])
